...
class Wholesale extends \Opencart\System\Engine\Controller {
    ...
    public function index(): void {
        ...
        $this->response->setOutput($this->load->view('sale/wholesale', $data));
    }
    ...
    public function getSalesByOrderData(): void {
        try {
            ...
            $results = $this->model_sale_wholesale->getDailyOrderSummary($filter);
            $total   = $this->model_sale_wholesale->getTotalOrderDays($filter);
    ...
    public function getSalesByProductData(): void {
        try {
            ...
            $results = $this->model_sale_wholesale->getDailyProductReport($filter);
            $total   = $this->model_sale_wholesale->getTotalDays($filter);
   ...
   public function getCustomerOrderHistory(): void {
    ...
    public function getSalesBySellerData(): void {
        try {
            ...
            $results = $this->model_sale_wholesale->getSellerSummary($filter);
            $total   = $this->model_sale_wholesale->getTotalSellers($filter);
    ...
    public function getSalesByCouponData(): void {
        try {
            ...
            $results = $this->model_sale_wholesale->getCouponSummary($filter);
            $total   = $this->model_sale_wholesale->getTotalCoupons($filter);
...
<th>Coupon</th><th>Discount</th>
            ...
            if ($tab === 'salesbytotal') {
                echo "<tr>
                    ...
                    <th>R Price</th><th>R Tax</th><th>R Total</th>
                    <th>S Price</th><th>S Tax</th><th>S Total</th>
                    <th>Profit</th><th>Cash</th><th>UPI</th><th>Due</th><th>Advance</th><th>Coupon</th><th>Discount</th>
                </tr>";
                ...
                foreach ($results as $r) {
                    ...
                    echo "<tr>
                        ...
                        <td>{$r['s_price']}</td><td>{$r['s_tax']}</td><td>{$r['s_price']}</td>
                        <td>{$profit}</td>
                        <td>{$r['cash']}</td><td>{$r['upi']}</td><td>{$r['due']}</td><td>{$r['advance']}</td><td>{$r['coupon']}</td><td>{$r['discount']}</td>   
                    </tr>";
                    $sr++;
                }
            }
    ...
    try {
        ...
        // ✅ CALL MODEL
        $result = $this->model_sale_wholesale->updateDueAmount(
            $order_id,
            $customer_id,
            $amount,
            $payment_type
