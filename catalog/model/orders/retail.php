...
class Retail extends \Opencart\System\Engine\Model {

    /* ================= TAB 1: SALES PRICE (per order) ================= */

    /**
     * Mirrors admin model getOrders() exactly.
     * Retail filter: agent.customer_group_id = 2
     * cash/upi: reduced by returnable_balance (GREATEST 0)
     */
   ...
   public function getDailyOrderSummary(array $data = []): array {
    ...
    $where = " WHERE agent.customer_group_id = 2
     ...
     * Per-order status-based CASE for all 8 columns: s_price, s_tax, s_total,
     * discount, cash, upi, advance, due; then outer GROUP BY date.
     * cash: status 6 = sub_total,  else cash_amount - returnable_balance (GREATEST 0)
     * upi:  status 6 = 0,          else upi_amount  - returnable_balance (GREATEST 0)
    ...
    public function getReport(array $data = []): array {
        ...
        FROM (
            SELECT
                ...
                /* discount: same for both statuses */
                CASE
     ...
     * Retail filter via agent (customer_group_id = 2).
     * Groups by date + phone + coupon_code.
   ...
   public function getCouponSummary(array $data = []): array {
    ...
    /**
     * Mirrors admin model getOrders() aggregation logic for a single order.
     * Retail filter: agent.customer_group_id = 2
     */
